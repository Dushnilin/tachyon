...
function validate_dns_settings(settings, sections, context) {
    ...
    if (length(main_servers) > 1 || length(bootstrap_servers) > 1) {
        ...
        for (let name in [ "dns_failure_threshold", "dns_recovery_threshold", "dns_failover_failure_threshold" ]) {
    ...
    // DoH/DoT/DoQ URLs (containing "://") and path-style DoH (e.g. "dns.example/dns-query")
    // are not valid here because the failover daemon treats fallback servers as plain UDP
    // upstreams only. Reject anything that contains "/" as the path separator.
...
function validate_runtime_mark_ranges_context(context) {
    ...
    if (fakeip_mark == null || outbound_mark == null)
        fail_validation("Tachyon marks contain invalid numeric constants. Aborted.");
