#!/usr/bin/env ucode

let fs = require("fs");

function as_string(value) {
    return value == null ? "" : "" + value;
}

function trim(str) {
    return replace(as_string(str), /^[ \t\r\n]+|[ \t\r\n]+$/g, "");
}

function write_json(value) {
    print(sprintf("%J", value), "\n");
}

// Convert <b 0x...> ucode binary literals to plain hex string
function to_hex(s) {
    s = replace(as_string(s), "<b 0x", "");
    s = replace(s, ">", "");
    return s;
}

function error_response(message) {
    write_json({
        success: false,
        message: as_string(message)
    });
}

function exec_output(command) {
    let pipe = fs.popen(command, "r");
    if (!pipe) return "";
    let data = pipe.read("all");
    pipe.close();
    return data == null ? "" : trim(data);
}

function contains(arr, val) {
    for (let x in arr) {
        if (x == val) return true;
    }
    return false;
}

function call_api(method, url, auth_header, body_file) {
    let resolve_ips = [ "162.159.192.1", "162.159.193.1", "188.114.98.1", "188.114.99.1" ];
    let proxy_opts = [ "", "-x socks5h://127.0.0.1:4534" ];
    let last_res = "";
    
    for (let proxy in proxy_opts) {
        for (let ip in resolve_ips) {
            let auth_str = auth_header != "" ? "-H 'Authorization: Bearer " + auth_header + "' " : "";
            let data_str = body_file != "" ? "-d @" + body_file + " " : "";
            let cmd = "curl -s --connect-timeout 6 --max-time 12 " + proxy + " --resolve api.cloudflareclient.com:443:" + ip + " -X " + method + " -H 'Content-Type: application/json' -H 'User-Agent: okhttp/3.12.1' " + auth_str + data_str + url;
            let res = exec_output(cmd);
            if (res != "") {
                try {
                    let parsed = json(res);
                    if (parsed) {
                        return parsed;
                    }
                } catch(e) {}
                last_res = res;
            }
        }
    }
    return last_res != "" ? { success: false, message: last_res } : null;
}

// Query current proxy selection from Clash API for automatic rate limit rotation if proxy is used
let original_server = "";
let available_servers = [];
let clash_data_str = exec_output("curl -s http://192.168.2.1:9090/proxies/MAIN-out");
if (clash_data_str != "") {
    try {
        let parsed = json(clash_data_str);
        if (parsed) {
            original_server = parsed.now || "";
            available_servers = parsed.all || [];
        }
    } catch(e) {}
}

function restore_original_server() {
    if (original_server != "") {
        let body = { name: original_server };
        let tmp_file = "/tmp/restore_clash.json";
        fs.writefile(tmp_file, sprintf("%J", body));
        exec_output("curl -s -X PUT -H 'Content-Type: application/json' -d @" + tmp_file + " http://192.168.2.1:9090/proxies/MAIN-out");
        fs.unlink(tmp_file);
    }
}

function switch_server(srv_name) {
    let body = { name: srv_name };
    let tmp_file = "/tmp/switch_clash.json";
    fs.writefile(tmp_file, sprintf("%J", body));
    exec_output("curl -s -X PUT -H 'Content-Type: application/json' -d @" + tmp_file + " http://192.168.2.1:9090/proxies/MAIN-out");
    fs.unlink(tmp_file);
    exec_output("sleep 1.5");
}

// 1. Generate WireGuard keys
let keypair_data = exec_output("/usr/bin/sing-box generate wg-keypair 2>/dev/null");
let private_key = null;
let public_key = null;

for (let line in split(keypair_data, "\n")) {
    let m_priv = match(line, /^PrivateKey:\s*(.+)$/);
    if (m_priv) private_key = trim(m_priv[1]);
    let m_pub = match(line, /^PublicKey:\s*(.+)$/);
    if (m_pub) public_key = trim(m_pub[1]);
}

// Fallback to wg genkey/pubkey if sing-box generate wg-keypair fails
if (!private_key || !public_key) {
    let priv = exec_output("wg genkey 2>/dev/null");
    if (priv != "") {
        private_key = priv;
        public_key = exec_output("echo '" + priv + "' | wg pubkey 2>/dev/null");
    }
}

if (!private_key || !public_key) {
    restore_original_server();
    error_response("Failed to generate WireGuard keypair");
    exit(1);
}

// 2. Perform registration
let install_id = "";
let fcm_token = "";
let tos = exec_output("date -u +'%Y-%m-%dT%H:%M:%SZ' 2>/dev/null") || "2026-07-17T20:52:00Z";
let serial_number = exec_output("head -c 8 /dev/urandom | hexdump -ve '1/1 \"%02x\"' 2>/dev/null") || "abcdef1234567890";

let reg_request = {
    key: public_key,
    install_id: install_id,
    fcm_token: fcm_token,
    tos: tos,
    model: "PC",
    serial_number: serial_number,
    locale: "en_US"
};

let tmp_req_file = exec_output("mktemp 2>/dev/null") || "/tmp/warp_req.json";
if (fs.writefile(tmp_req_file, sprintf("%J", reg_request)) == null) {
    restore_original_server();
    error_response("Failed to write temporary registration request file");
    exit(1);
}

let api_prefixes = [
    "v0i1909051800",
    "v0a1909051800",
    "v0b1909051800",
    "v0c1909051800",
    "v0d1909051800",
    "v0e1909051800",
    "v0f1909051800",
    "v0g1909051800",
    "v0h1909051800",
    "v0i1604021500",
    "v0a1604021500"
];

let response_data = null;
let last_error = "";
let chosen_prefix = "";
let server_try_index = 0;

for (let prefix in api_prefixes) {
    let url = "https://api.cloudflareclient.com/" + prefix + "/reg";
    let parsed = call_api("POST", url, "", tmp_req_file);
    
    if (!parsed) {
        last_error = "Timeout or connection failure (prefix: " + prefix + ")";
        continue;
    }
    
    if (parsed.success && parsed.result) {
        response_data = parsed.result;
        chosen_prefix = prefix;
        break;
    }
    
    let msg = parsed.message || (parsed.errors ? parsed.errors[0].message : "Unsuccessful registration");
    let code = parsed.errors ? parsed.errors[0].code : null;
    
    if (code == 1015 || index(as_string(msg), "1015") >= 0) {
        // Rotate proxy server if available
        if (length(available_servers) > 0 && server_try_index < 4) {
            let next_srv = available_servers[(server_try_index + 1) % length(available_servers)];
            server_try_index++;
            switch_server(next_srv);
            
            // Retry request with rotated proxy server
            parsed = call_api("POST", url, "", tmp_req_file);
            if (parsed && parsed.success && parsed.result) {
                response_data = parsed.result;
                chosen_prefix = prefix;
                break;
            }
            msg = parsed ? (parsed.message || (parsed.errors ? parsed.errors[0].message : "Unsuccessful registration")) : "Timeout after proxy rotation";
            code = parsed && parsed.errors ? parsed.errors[0].code : null;
        }
        
        if (code == 1015 || index(as_string(msg), "1015") >= 0) {
            msg = "Rate limit exceeded (Cloudflare error 1015). Cloudflare blocks registrations from this IP. Please switch your active server in the Tachyon Dashboard tab and try again.";
        }
    }
    
    last_error = msg;
}
fs.unlink(tmp_req_file);

if (!response_data) {
    restore_original_server();
    error_response("Failed to register with Cloudflare WARP API: " + last_error);
    exit(1);
}

// 3. Make sure warp_enabled is true
if (!response_data.warp_enabled) {
    let patch_body = { warp_enabled: true };
    let tmp_patch = exec_output("mktemp 2>/dev/null") || "/tmp/warp_patch.json";
    fs.writefile(tmp_patch, sprintf("%J", patch_body));
    
    let url = "https://api.cloudflareclient.com/" + chosen_prefix + "/reg/" + response_data.id;
    call_api("PATCH", url, response_data.token, tmp_patch);
    fs.unlink(tmp_patch);
}

// 4. Optionally apply WARP+ license key if provided as first arg
let license_key = ARGV[0] || "";
if (license_key != "") {
    let patch_body = { license_key: license_key };
    let tmp_patch = exec_output("mktemp 2>/dev/null") || "/tmp/warp_patch.json";
    fs.writefile(tmp_patch, sprintf("%J", patch_body));
    
    let url = "https://api.cloudflareclient.com/" + chosen_prefix + "/reg/" + response_data.id;
    call_api("PATCH", url, response_data.token, tmp_patch);
    fs.unlink(tmp_patch);
}

// Restore original proxy server selection
restore_original_server();

// 5. Select fastest endpoint IP using TCP latency probes (via curl)
let probe_ips = [
    "162.159.192.1", "162.159.192.6",
    "162.159.193.1", "162.159.193.6",
    "188.114.98.1", "188.114.98.6",
    "188.114.99.1", "188.114.99.6",
    "8.6.112.0"
];

// Try to resolve engage.cloudflareclient.com and add resolved IPs
let resolved_ips = exec_output("nslookup engage.cloudflareclient.com 2>/dev/null | grep -E 'Address[ 0-9]*:' | awk '{print $2}'");
for (let ip in split(resolved_ips, "\n")) {
    ip = trim(ip);
    if (ip != "" && index(ip, "127.0.0.") < 0 && index(ip, ":") < 0 && !contains(probe_ips, ip)) {
        push(probe_ips, ip);
    }
}

let best_ip = "162.159.192.1";
let best_time = 9999.0;

for (let ip in probe_ips) {
    ip = trim(ip);
    if (ip == "") continue;
    
    let cmd = "curl -w '%{time_connect}' -o /dev/null -s --max-time 1.0 https://" + ip;
    let t_str = exec_output(cmd);
    let t = (t_str || "9999.0") * 1.0;
    if (t > 0.0 && t < best_time) {
        best_time = t;
        best_ip = ip;
    }
}

let warp_ports = [
    854, 859, 864, 878, 880, 890, 891, 894, 903, 908,
    928, 934, 939, 942, 943, 945, 946, 955, 968, 987,
    988, 1002, 1010, 1014, 1018, 1070, 1074, 1180, 1387, 1843,
    2371, 2506, 3138, 3476, 3581, 3854, 4177, 4198, 4233, 5279,
    5956, 7103, 7152, 7156, 7281, 7559, 8319, 8742, 8854, 8886, 2408
];
let random_port = warp_ports[time() % length(warp_ports)];

// 6. Formulate CPS packets
let warp_cps_payloads = [
    "<b >",
    "<b >",
    "<b >"
];

function generate_random_hex(len) {
    return exec_output("head -c " + int(len) + " /dev/urandom | hexdump -ve '1/1 \"%02x\"' 2>/dev/null") || "";
}

function generate_extra_cps() {
    let rand_val = (time() + int(exec_output("echo $PPID 2>/dev/null") || "1")) % 3 + 1;
    let result = "";
    for (let i = 0; i < rand_val; i++) {
        let length = (time() + i * 7) % 48 + 16;
        result += "<b 0x" + generate_random_hex(length) + ">";
    }
    return result;
}

let i1 = to_hex(warp_cps_payloads[time() % length(warp_cps_payloads)]);
let i2 = to_hex(generate_extra_cps());
let i3 = to_hex(generate_extra_cps());
let i4 = to_hex(generate_extra_cps());
let i5 = to_hex(generate_extra_cps());

// 7. Output result
write_json({
    success: true,
    local_address: response_data.config.interface.addresses.v4 + "/32 " + response_data.config.interface.addresses.v6 + "/128",
    private_key: private_key,
    peer_public_key: response_data.config.peers[0].public_key,
    server_address: best_ip,
    server_port: random_port,
    
    awg_jc: 4,
    awg_jmin: 40,
    awg_jmax: 70,
    awg_s1: 0, awg_s2: 0, awg_s3: 0, awg_s4: 0,
    awg_h1: 1, awg_h2: 2, awg_h3: 3, awg_h4: 4,
    
    awg_i1: i1,
    awg_i2: i2,
    awg_i3: i3,
    awg_i4: i4,
    awg_i5: i5
});
