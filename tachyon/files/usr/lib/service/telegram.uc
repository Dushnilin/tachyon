...
function process_command(token, chat_id, text) {
    ...
    if (cmd == "/start" || cmd == "/help") {
        let help_text = "🤖 *Tachyon Bot* — Панель управления роутером\n\n" +
            ...
            "🔄 /restart — Перезапустить Tachyon\n" +
            "📋 /logs — Посмотреть последние логи\n";
        send_message(token, chat_id, help_text, "Markdown");
    }
    else if (cmd == "/status") {
        ...
        send_message(token, chat_id, status_text, "Markdown");
    }
    else if (cmd == "/doctor") {
        ...
        send_message(token, chat_id, "🩺 *Результаты Tachyon Doctor:*\n\n```\n" + report + "\n```", "Markdown");
    }
    else if (cmd == "/restart") {
        send_message(token, chat_id, "🔄 *Перезапускаю службы Tachyon...*");
        let restart_status = command_status("/usr/bin/tachyon restart");
        if (restart_status == 0) {
            send_message(token, chat_id, "✅ *Перезапуск выполнен успешно!*");
        } else {
            send_message(token, chat_id, "❌ *Ошибка при перезапуске служб.*");
        }
    }
    else if (cmd == "/logs") {
        let logs_res = command_capture("logread -e tachyon | tail -n 25");
        let logs = logs_res.output || "Логи отсутствуют.";
        send_message(token, chat_id, "📋 *Последние логи Tachyon:*\n\n```\n" + logs + "\n```", "Markdown");
    }
    else {
        send_message(token, chat_id, "⚠️ Неизвестная команда. Введите /help для списка команд.");
    }
}
