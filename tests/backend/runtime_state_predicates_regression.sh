...
state_ucode() {
  ucode -L "$PODKOP_LIB" "$STATE_UC" "$@"
}

nft_ucode() {
  ucode -L "$PODKOP_LIB" "$NFT_UC" "$@"
}
...
cat >"$WORK_DIR/service-dns-state.json" <<'JSON'
{
  ...
  "dhcp_dnsmasq": {
    ...
    "noresolv": "1",
    "cachesize": "0",
    "podkop_server": [ "127.0.0.42#53" ],
    "podkop_noresolv": "0",
    "podkop_cachesize": "1500"
  },
...
0
[dhcp.@dnsmasq[0].podkop_server]
127.0.0.42#53
[dhcp.@dnsmasq[0].podkop_noresolv]
0
[dhcp.@dnsmasq[0].podkop_cachesize]
1500
[dhcp.podkop_plus.present]
1
EOF_DNSMASQ_SIG
...
0
[settings.download_subscriptions_via_proxy]
1
[settings.download_components_via_proxy]
0
[settings.download_lists_via_proxy_section]
proxy1
[rule.proxy1.action]
proxy
...
[rule.proxy1.enable_udp_over_tcp]
1
...
[rule.proxy1.subscription_update_enabled]
1
...
[rule.vpn1.interface]
wg0
[rule.vpn1.domain_resolver_enabled]
1
[rule.vpn1.domain_resolver_dns_type]
udp
[rule.vpn1.domain_resolver_dns_server]
8.8.8.8
...
printf 'runtime state predicate regression checks passed\n'
