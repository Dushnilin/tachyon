...
assert_file "$WORK_DIR/default.txt" "$(printf '%s\n%s\n%s\n%s\n%s\n%s' "$default_ua" v2rayN Happ Hiddify Clash.Meta ClashMetaForAndroid)"

cache_ucode write-user-agent-candidates "$WORK_DIR/preferred.txt" "" Hiddify "$default_ua"
assert_file "$WORK_DIR/preferred.txt" "$(printf '%s\n%s\n%s\n%s\n%s\n%s' "$default_ua" v2rayN Hiddify Happ Clash.Meta ClashMetaForAndroid)"
...
cache_ucode user-agent-matches-config "" Hiddify "$default_ua" >/dev/null ||
