...
assert_eq "*/30 * * * * /usr/bin/podkop list_update_if_due # list" \
  "$(updates_ucode list-update-cron-job 30m /usr/bin/podkop '# list')" \
  "list update cron job"
assert_eq "0 */2 * * * /usr/bin/podkop subscription_update_if_due # subscription" \
  "$(updates_ucode subscription-update-cron-job 7200 /usr/bin/podkop '# subscription')" \
...
if updates_ucode list-update-cron-job invalid /usr/bin/podkop '# list' >/dev/null 2>&1; then
  fail "invalid list update interval should fail"
fi
if updates_ucode subscription-update-cron-job 0 /usr/bin/podkop '# subscription' >/dev/null 2>&1; then
...
if (value.crontab !== "0 1 * * * /bin/true # keep\n") {
  console.error("invalid plan should remove old podkop cron jobs only", JSON.stringify(value.crontab));
  process.exit(1);
}
...
printf 'updates due regression checks passed\n'
