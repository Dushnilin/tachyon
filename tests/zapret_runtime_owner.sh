...
ZAPRET2_RUNTIME="$TACHYON_LIB/providers/zapret2/runtime.uc"
ZAPRET_COMMON="$TACHYON_LIB/providers/zapret/common.uc"
ZAPRET2_COMMON="$TACHYON_LIB/providers/zapret2/common.uc"
NFQUEUE_RUNTIME="$TACHYON_LIB/providers/nfqueue/runtime.uc"
NFQUEUE_CHECK="$TACHYON_LIB/providers/nfqueue/check.uc"
...
if grep -R -n -E 'providers\.zapret\.|providers/zapret/' "$TACHYON_LIB/providers/zapret2" >/dev/null 2>&1; then
...
status_json="$(TACHYON_CONFIG_NAME=tachyon-definitely-missing ucode -L "$TACHYON_LIB" "$ZAPRET_RUNTIME" status)"
...
check_json="$(ucode -L "$TACHYON_LIB" "$ZAPRET2_RUNTIME" check)"
...
ucode -L "$TACHYON_LIB" -- "$ZAPRET2_VALIDATOR" validate-json nfqws2 '--name tachyon --intercept=1' >/dev/null ||
