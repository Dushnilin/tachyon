...
if node "$COMPARE" "$WORK_DIR/expected.json" "$WORK_DIR/different-array-order.json" >/tmp/tachyon-semantic-json.out 2>&1; then
  fail "array order mismatch should be rejected"
fi

grep -Fq '$.outbounds[0].tag: "first" != "second"' /tmp/tachyon-semantic-json.out ||
