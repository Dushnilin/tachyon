...
if grep -R -n "ui_runtime.sh" "$TACHYON_FILES" >/dev/null 2>&1; then
  fail "runtime files must not reference ui_runtime.sh"
fi

ui_runtime_shell_symbols='ui_runtime_|tachyon_fast_get_ui_state|TACHYON_UI_RUNTIME|load_ui_runtime'
if grep -R -n -E "$ui_runtime_shell_symbols" "$TACHYON_BIN" "$TACHYON_INIT" "$TACHYON_FILES/usr/lib" --include='*.sh' >/dev/null 2>&1; then
  fail "ui_runtime shell symbols must not remain in runtime shell"
fi

if grep -R -n '^get_ui_capabilities()' "$TACHYON_FILES/usr/lib" --include='*.sh' >/dev/null 2>&1; then
...
grep -Fq '"tachyon-stably-running"' "$UI_UC" ||
...
export TACHYON_UI_LATENCY_ACTION_DIR="$TACHYON_UI_STATE_DIR/latency-actions"
export TACHYON_UI_COMPONENT_ACTION_DIR="$TACHYON_UI_STATE_DIR/component-actions"
export TACHYON_UI_SUBSCRIPTION_ACTION_DIR="$TACHYON_UI_STATE_DIR/subscription-actions"

latency_start="$(
  TACHYON_BIN=/bin/true \
...
latency_async_state="$TACHYON_UI_LATENCY_ACTION_DIR/$latency_job_id.json"
...
service_state="$TACHYON_UI_SERVICE_ACTION_DIR/$job_id.json"
...
TACHYON_UI_SERVICE_ACTION_TIMEOUT_SECONDS=1 \
TACHYON_UI_SERVICE_ACTION_SETTLE_SECONDS=0 \
TACHYON_SERVICE_INIT=/bin/true \
TACHYON_BIN=/bin/true \
TACHYON_LIB="$TACHYON_FILES/usr/lib" \
