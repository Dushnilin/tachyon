...
export TACHYON_DEADLINE_HELPER_PATH="$deadline_helper"
export TACHYON_INSTALLER_DEADLINE_HELPER="$deadline_helper"
export TACHYON_INSTALLER_COMMAND_RESULT="$WORK_DIR/installer-command"
...
grep -Fq '/usr/bin/tachyon component_action sing_box "$action"' "$INSTALLER" ||
  fail "selected sing-box install must delegate to tachyon component_action"
...
if grep -n -E 'rm -f /var/luci-indexcache|rm -f /tmp/luci-indexcache|/etc/init\.d/rpcd[[:space:]]+reload|/etc/init\.d/tachyon[[:space:]]+(start|stop|disable|enable|restart)|/etc/init\.d/tachyon[[:space:]]+(stop|disable)' "$INSTALLER" >/dev/null 2>&1; then
...
release_json='{"tag_name":"0.0.1","assets":[{"name":"tachyon_0.0.1.ipk","browser_download_url":"https://example.com/tachyon.ipk"}]}'
printf '%s' "$release_json" | ucode "$helper" release-asset-url backend ipk | grep -Fxq 'https://example.com/tachyon.ipk' ||
  fail "embedded helper must resolve the exact three-part Tachyon package name"
if printf '%s' '{"tag_name":"0.0.1","assets":[{"name":"tachyon_0.0.1_all.ipk","browser_download_url":"https://example.com/old.ipk"}]}' |
  ucode "$helper" release-asset-url backend ipk | grep -q .; then
  fail "embedded helper must reject package names outside the Tachyon release format"
fi
...
case "$1" in
  list-installed)
    ...
    if [ -n "${TACHYON_INSTALLER_FAKE_LEGACY_BACKEND:-}" ]; then
      printf '%s\n' \
        "$TACHYON_INSTALLER_FAKE_LEGACY_BACKEND - 1.0" \
        "luci-app-$TACHYON_INSTALLER_FAKE_LEGACY_BACKEND - 1.0" \
        "luci-i18n-$TACHYON_INSTALLER_FAKE_LEGACY_BACKEND-ru - 1.0"
    fi
    ;;
  remove)
    shift
    [ "$1" = "--force-depends" ] && shift
    printf '%s\n' "$1" >> "$TACHYON_INSTALLER_OPKG_LOG"
    [ "${TACHYON_INSTALLER_FAIL_REMOVE:-}" != "$1" ] || exit 1
    ;;
esac
exit 0
SH
...
#!/usr/bin/env sh
printf '%s\n' "$1" >> "$TACHYON_INSTALLER_INIT_LOG"
case "$1" in
  enabled|status|running)
    printf '%s\n' "$$" >> "$TACHYON_INSTALLER_HANG_PID_LOG"
    trap '' TERM
    sh -c '
      trap "" TERM
      printf "%s\n" "$$" >> "$TACHYON_INSTALLER_HANG_PID_LOG"
...
ORPHAN_PROBE_PID="$(
  TACHYON_INSTALLER_INIT_LOG="$WORK_DIR/hanging-init.log" \
  TACHYON_INSTALLER_HANG_PID_LOG="$HANG_PID_LOG" \
...
TACHYON_INSTALLER_INIT="$WORK_DIR/hanging-init" \
TACHYON_INSTALLER_BIN="$WORK_DIR/hanging-bin" \
TACHYON_INSTALLER_LIB="$WORK_DIR/missing-tachyon-lib" \
TACHYON_INSTALLER_UCI_DEFAULTS="$WORK_DIR/missing-uci-defaults" \
TACHYON_INSTALLER_LUCI_VIEW="$WORK_DIR/missing-luci-view" \
TACHYON_INSTALLER_MENU_JSON="$WORK_DIR/missing-menu.json" \
TACHYON_INSTALLER_ACL_JSON="$WORK_DIR/missing-acl.json" \
TACHYON_INSTALLER_RU_LMO="$WORK_DIR/missing-ru.lmo" \
TACHYON_INSTALLER_EN_LMO="$WORK_DIR/missing-en.lmo" \
TACHYON_INSTALLER_RU_LUA="$WORK_DIR/missing-ru.lua" \
TACHYON_INSTALLER_EN_LUA="$WORK_DIR/missing-en.lua" \
TACHYON_INSTALLER_LEGACY_BASE_INIT="$WORK_DIR/missing-original-init" \
TACHYON_INSTALLER_LEGACY_BRAND="$LEGACY_BRAND" \
TACHYON_INSTALLER_LEGACY_BACKEND="$LEGACY_BACKEND" \
TACHYON_INSTALLER_RC_DIR="$WORK_DIR/rc.d" \
TACHYON_INSTALLER_START_RETRY_FILE="$WORK_DIR/start.retry" \
TACHYON_INSTALLER_START_RETRY_PID_FILE="$WORK_DIR/start-retry.pid" \
TACHYON_INSTALLER_ORPHAN_PPID="$ORPHAN_PROBE_PPID" \
TACHYON_INSTALLER_SERVICE_PROBE_TIMEOUT=1 \
TACHYON_INSTALLER_SERVICE_ACTION_TIMEOUT=3 \
TACHYON_INSTALLER_INIT_LOG="$WORK_DIR/hanging-init.log" \
TACHYON_INSTALLER_HANG_PID_LOG="$HANG_PID_LOG" \
...
grep -Fxq 'TACHYON_WAS_ENABLED=1' "$WORK_DIR/hanging-state.env" ||
...
printf '%s\n' '1' |
  PATH="$WORK_DIR:$PATH" \
  TACHYON_INSTALLER_OPKG_LOG="$WORK_DIR/opkg.log" \
  TACHYON_INSTALLER_INIT="$WORK_DIR/missing-tachyon-init" \
  TACHYON_INSTALLER_BIN="$WORK_DIR/missing-tachyon-bin" \
  TACHYON_INSTALLER_LIB="$WORK_DIR/missing-tachyon-lib" \
  TACHYON_INSTALLER_UCI_DEFAULTS="$WORK_DIR/missing-uci-defaults" \
  TACHYON_INSTALLER_LUCI_VIEW="$WORK_DIR/missing-luci-view" \
  TACHYON_INSTALLER_MENU_JSON="$WORK_DIR/missing-menu.json" \
...
TACHYON_INSTALLER_BIN="$WORK_DIR/missing-tachyon-bin" \
TACHYON_INSTALLER_LIB="$WORK_DIR/missing-tachyon-lib" \
TACHYON_INSTALLER_UCI_DEFAULTS="$WORK_DIR/uci-defaults" \
TACHYON_INSTALLER_LUCI_VIEW="$WORK_DIR/luci-view" \
TACHYON_INSTALLER_MENU_JSON="$WORK_DIR/menu.json" \
TACHYON_INSTALLER_ACL_JSON="$WORK_DIR/acl.json" \
TACHYON_INSTALLER_RU_LMO="$WORK_DIR/ru.lmo" \
TACHYON_INSTALLER_EN_LMO="$WORK_DIR/en.lmo" \
TACHYON_INSTALLER_RU_LUA="$WORK_DIR/ru.lua" \
TACHYON_INSTALLER_EN_LUA="$WORK_DIR/en.lua" \
TACHYON_INSTALLER_LEGACY_BASE_INIT="$WORK_DIR/missing-original-init" \
TACHYON_INSTALLER_LEGACY_BRAND="$LEGACY_BRAND" \
TACHYON_INSTALLER_LEGACY_BACKEND="$LEGACY_BACKEND" \
TACHYON_INSTALLER_LEGACY_CONFIG_ALT="$LEGACY_CONFIG_ALT" \
TACHYON_INSTALLER_LEGACY_INIT="$WORK_DIR/legacy-init" \
TACHYON_INSTALLER_LEGACY_BIN="$WORK_DIR/legacy-bin" \
TACHYON_INSTALLER_LEGACY_LIB="$WORK_DIR/legacy-lib" \
...
tachyon_persistent="$WORK_DIR/tachyon-persistent"
...
mkdir -p \
  ...
  "$tachyon_persistent/tailscale/server-existing" \
...
printf '%s\n' 'current-tachyon-identity' >"$tachyon_persistent/tailscale/server-existing/node.key"
...
TACHYON_INSTALLER_LEGACY_BACKEND="$LEGACY_BACKEND" \
TACHYON_INSTALLER_LEGACY_CONFIG_ALT="$LEGACY_CONFIG_ALT" \
TACHYON_INSTALLER_LEGACY_CONFIG="$legacy_config" \
TACHYON_INSTALLER_LEGACY_CONFIG_FILE_ALT="$legacy_config_alt" \
TACHYON_INSTALLER_LEGACY_PERSISTENT_DIR="$legacy_persistent" \
TACHYON_INSTALLER_PERSISTENT_DIR="$tachyon_persistent" \
TACHYON_INSTALLER_LEGACY_RUNTIME_DIR="$legacy_runtime" \
TACHYON_INSTALLER_LEGACY_TMP_DIR="$legacy_tmp" \
TACHYON_INSTALLER_LEGACY_TMP_ALT_DIR="$legacy_tmp_alt" \
TACHYON_INSTALLER_LEGACY_BASE_CONFIG="$legacy_base_config" \
TACHYON_INSTALLER_LEGACY_BASE_PERSISTENT_DIR="$legacy_base_persistent" \
TACHYON_INSTALLER_LEGACY_BASE_RUNTIME_DIR="$legacy_base_runtime" \
TACHYON_INSTALLER_LEGACY_BASE_TMP_DIR="$legacy_base_tmp" \
TACHYON_INSTALLER_LEGACY_BASE_INIT="$legacy_base_init" \
TACHYON_INSTALLER_LEGACY_BASE_BIN="$legacy_base_bin" \
TACHYON_INSTALLER_LEGACY_BASE_LIB="$legacy_base_lib" \
TACHYON_INSTALLER_LEGACY_BASE_UCI_DEFAULTS="$legacy_base_uci_defaults" \
TACHYON_INSTALLER_LEGACY_BASE_LUCI_VIEW="$legacy_base_luci_view" \
TACHYON_INSTALLER_LEGACY_BASE_MENU_JSON="$legacy_base_menu" \
TACHYON_INSTALLER_LEGACY_BASE_ACL_JSON="$legacy_base_acl" \
TACHYON_INSTALLER_LEGACY_BASE_I18N="$legacy_base_i18n" \
TACHYON_INSTALLER_LEGACY_TMP_PACKAGE_GLOB="$WORK_DIR/*${LEGACY_BRAND}*" \
TACHYON_INSTALLER_LEGACY_SCAN_ROOTS="$legacy_scan_root" \
  ucode "$helper" installer-finalize-legacy
[ "$(cat "$tachyon_persistent/tailscale/server-new/node.key")" = 'legacy-node-identity' ] ||
  fail "installer finalization must migrate legacy Tailscale node identity"
[ "$(cat "$tachyon_persistent/tailscale/server-existing/node.key")" = 'current-tachyon-identity' ] ||
  fail "installer finalization must not overwrite existing Tachyon Tailscale state"
...
tachyon_persistent_file="$WORK_DIR/tachyon-persistent-file"
touch "$tachyon_persistent_file"
if TACHYON_INSTALLER_LEGACY_BRAND="$LEGACY_BRAND" \
  TACHYON_INSTALLER_LEGACY_BACKEND="$LEGACY_BACKEND" \
  TACHYON_INSTALLER_LEGACY_CONFIG_ALT="$LEGACY_CONFIG_ALT" \
  TACHYON_INSTALLER_LEGACY_CONFIG="$legacy_config" \
  TACHYON_INSTALLER_LEGACY_CONFIG_FILE_ALT="$legacy_config_alt" \
  TACHYON_INSTALLER_LEGACY_PERSISTENT_DIR="$legacy_persistent" \
  TACHYON_INSTALLER_PERSISTENT_DIR="$tachyon_persistent_file" \
  TACHYON_INSTALLER_LEGACY_RUNTIME_DIR="$legacy_runtime" \
  TACHYON_INSTALLER_LEGACY_TMP_DIR="$legacy_tmp" \
  TACHYON_INSTALLER_LEGACY_TMP_ALT_DIR="$legacy_tmp_alt" \
  TACHYON_INSTALLER_LEGACY_BASE_CONFIG="$legacy_base_config" \
  ...
  TACHYON_INSTALLER_LEGACY_BASE_ACL_JSON="$legacy_base_acl" \
  TACHYON_INSTALLER_LEGACY_BASE_I18N="$legacy_base_i18n" \
  TACHYON_INSTALLER_LEGACY_TMP_PACKAGE_GLOB="$WORK_DIR/missing-${LEGACY_BRAND}*" \
  TACHYON_INSTALLER_LEGACY_SCAN_ROOTS="$legacy_scan_root" \
...
write_fake_service_init "$WORK_DIR/tachyon-init"
touch "$WORK_DIR/luci-indexcache.one" "$WORK_DIR/luci-indexcache.two"
: >"$WORK_DIR/init.log"
TACHYON_INSTALLER_INIT="$WORK_DIR/tachyon-init" \
TACHYON_INSTALLER_RPCD_INIT="$WORK_DIR/missing-rpcd" \
TACHYON_INSTALLER_LUCI_CACHE_GLOBS="$WORK_DIR/luci-indexcache*" \
TACHYON_INSTALLER_LATEST_VERSION_CACHE="$WORK_DIR/latest.cache" \
TACHYON_INSTALLER_SYSTEM_INFO_CACHE="$WORK_DIR/system-info.json" \
TACHYON_INSTALLER_SERVER_COUNTRY_CACHE="$WORK_DIR/server-country.json" \
TACHYON_INSTALLER_SING_BOX_VERSION_CACHE="$WORK_DIR/sing-box-version" \
TACHYON_INSTALLER_TMP_SYSTEM_INFO_CACHE="$WORK_DIR/tmp-system-info.json" \
