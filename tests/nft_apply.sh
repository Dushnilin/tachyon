...
assert_contains "$NFT_LOG" $'nft\tadd\tchain\tinet\tTachyonTable\tpredefrag\t{ type filter hook output priority -401; policy accept; }' "runtime predefrag chain runs before conntrack"
assert_contains "$NFT_LOG" $'nft\tadd\tchain\tinet\tTachyonTable\tpredefrag_prerouting\t{ type filter hook prerouting priority -401; policy accept; }' "runtime predefrag prerouting chain runs before conntrack"
assert_contains "$NFT_LOG" $'nft\tadd\trule\tinet\tTachyonTable\tpredefrag\tmeta\tmark\t&\t0x60000000\t!=\t0\tnotrack\tcounter' "runtime notrack desync mark rule"
