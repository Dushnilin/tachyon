...
const syncDirs = [
  {
    ...
    remote:
      process.env.REMOTE_DIR_FE ?? '/www/luci-static/resources/view/tachyon',
  },
  {
    local: path.resolve(
      process.env.LOCAL_DIR_BIN ?? '../tachyon/files/usr/bin/',
    ),
    remote: process.env.REMOTE_DIR_BIN ?? '/usr/bin/',
  },
  {
    local: path.resolve(
      process.env.LOCAL_DIR_LIB ?? '../tachyon/files/usr/lib/',
    ),
    remote: process.env.REMOTE_DIR_LIB ?? '/usr/lib/tachyon/',
  },
  {
    local: path.resolve(
      process.env.LOCAL_DIR_INIT ?? '../tachyon/files/etc/init.d/',
    ),
